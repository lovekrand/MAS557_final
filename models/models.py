...
from models.utils import get_filter
...
class MWT_CZ1d_multi(nn.Module):
    def __init__(self,
        ...
        self.AvgPool2d = nn.AdaptiveAvgPool2d(k)
        self.Attention_mat_high = nn.LazyConv2d(c,k)
        self.Attention_mat_low = nn.LazyConv2d(c,k)
        self.flatten_high = nn.Flatten()
        self.flatten_low = nn.Flatten()
...
#         decompose
        ...
        # Fuse
        Fuse = Ud + Us
        Fuse = self.AvgPool2d(Fuse)
        kernel_high = self.Attention_mat_high(Fuse)
        kernel_low = self.Attention_mat_low(Fuse)
        Us = torch.matmul(Us, kernel_high)
        Ud = torch.matmul(Ud, kernel_low)

#        reconstruct           
        for i in range((ns-self.L)//2+(ns-self.L)//2-1,-1,-1):
            x = x + Us[i]
            x = torch.cat((x, Ud[i]), -1)
            x = self.evenOdd(x)
        return x
